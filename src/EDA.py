...
import os
os.makedirs("../fig/original_cohort_eda", exist_ok=True) 
...
cursor.execute("SELECT sc.*, p.gender_source_value,p.location_id,p.year_of_birth FROM work_rizvanov_a263.stroke_cohort_dementia_and_aphasia_test sc inner join omop_cdm_53_pmtx_202203.person p on sc.person_id = p.person_id;")
...
plt.savefig("../fig/stroke_cts_by_yob.jpg")
print(f"Made stroke_cts_by_yob.jpg")
...
plt.grid(False)
plt.tight_layout()  # Ensure everything fits within the plot
plt.savefig("../fig/original_cohort_eda/stroke_ct_by_yob_gender.jpg")
print(f"Made stroke_ct_by_yob_gender.jpg")

#Fig 3: Stroke counts by state


cursor.execute("SELECT sc.*, p.gender_source_value,p.location_id,p.year_of_birth, l.* FROM work_rizvanov_a263.stroke_cohort_dementia_and_aphasia_test sc \
...
plt.grid(False)
plt.tight_layout()  # Ensure everything fits within the plot
plt.savefig("../fig/original_cohort_eda/stroke_ct_by_state.jpg")
print(f"Made stroke_ct_by_state.jpg")
...
(   \
select * from work_rizvanov_a263.stroke_cohort_dementia_and_aphasia_test s \
...
plt.savefig("../fig/original_cohort_eda/visit_ct_specialty.jpg")
print(f"Made isit_ct_specialty.jpg")
...
person_classification AS (
    ...
    FROM work_rizvanov_a263.stroke_cohort_dementia_and_aphasia_test sc
...
plt.savefig("../fig/original_cohort_eda/SLP_visits.jpg", bbox_inches ="tight")
print(f"Made SLP_visits.jp")

# Fig 6: Histogram showing counts of patients by the number of SLP visits
cursor.execute("""select count(*), sc.person_id from work_rizvanov_a263.stroke_cohort_dementia_and_aphasia_test sc
...
plt.savefig("../fig/original_cohort_eda/SLP_hist.jpg")
plt.close('all')
...
#plt.xticks(range(int(result['count'].min()), int(result['count'].max()), tick_interval), rotation=45)
plt.yscale('log')
plt.savefig("../fig/original_cohort_eda/SLP_hist_y_log_scale.jpg")
print(f"Made SLP_hist_y_log_scale.jpg")

#Fig 8: count of discharge codes

cursor.execute ("""select a.*,c.concept_name  from 
(select count(*), vo.discharge_to_concept_id from work_rizvanov_a263.stroke_cohort_dementia_and_aphasia_test scwa inner join
...
plt.tight_layout()
plt.savefig("../fig/original_cohort_eda/discharge_location.jpg")
print(f"Made discharge_location.jpg")